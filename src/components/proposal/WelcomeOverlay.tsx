  ...
  return (
    <AnimatePresence>
      {isVisible && (
        ...
        >
          <div className="max-w-4xl px-6 text-center relative w-full">
            {/* Main Welcome Text */}
          ...
          {/* Skip Button - Center Bottom - Appears after typing finishes */}
          <motion.button
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: showButton ? 1 : 0, y: showButton ? 0 : 20 }}
            transition={{ duration: 0.6 }}
            onClick={handleClose}
            className="absolute bottom-8 left-1/2 -translate-x-1/2 w-14 h-14 rounded-full bg-red-600 border-2 border-white/30 hover:border-white hover:bg-red-700 shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center group hover:scale-110"
          >
